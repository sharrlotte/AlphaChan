...
public class PageTable extends Table {
    ...
    public EmbedBuilder addPageFooter(EmbedBuilder value) {
        if (showPageNumber)
            return value.setFooter((pageNumber + 1) + "/" + getMaxPage());
        return value;
    }
    ...
    public void onPrepareTable(MessageEditAction action) {
        EmbedBuilder page = getCurrentPage();
        if (getMaxPage() <= 0 || page == null) {
            MessageHandler.sendMessage(getEventTextChannel(), "No data", 10);
            return;
        }

        action.setEmbeds(MessageHandler.getFormatEmbedBuilder(getEventGuild(), page).build());
    }
}
